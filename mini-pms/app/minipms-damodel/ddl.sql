...
CREATE TABLE pms_member (
  ...
  tell       VARCHAR(30)  NULL     COMMENT '전화', -- 전화
...
CREATE TABLE pms_board (
  ...
  created_df DATETIME     NOT NULL DEFAULT now() COMMENT '등록일', -- 등록일
...
CREATE TABLE pms_board_file (
  board_file_no INTEGER      NOT NULL COMMENT '게시판첨부파일번호', -- 게시판첨부파일번호
  board_no      INTEGER      NOT NULL COMMENT '게시판번호', -- 게시판번호
...
ALTER TABLE pms_board_file
  ADD CONSTRAINT PK_pms_board_file -- 게시판첨부파일 기본키
    PRIMARY KEY (
      board_file_no -- 게시판첨부파일번호
    );

ALTER TABLE pms_board_file
  MODIFY COLUMN board_file_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '게시판첨부파일번호';
...
-- 게시판첨부파일
ALTER TABLE pms_board_file
  ADD CONSTRAINT FK_pms_board_TO_pms_board_file -- 게시판 -> 게시판첨부파일
    FOREIGN KEY (
      board_no -- 게시판번호
    )
    REFERENCES pms_board ( -- 게시판
